...
;
 ...
 HoughCircles( tar1, circles, CV_HOUGH_GRADIENT, 4, tar1.rows/15, 300, 200, tar1.rows/30, tar1.rows/16 );
  // HoughCircles( src_gray, circles, CV_HOUGH_GRADIENT, 4, src_gray.rows/2, 300, 200, src_gray.cols/4, src_gray.cols/2 );
   qDebug()<<"done";
    for(std::vector<cv::Vec3f>::iterator it = circles.begin(); it != circles.end(); ++it) {
        qDebug()<<"x"<<(*it).val[0]<<" y"<<(*it).val[1];
    }
   ...
   for( size_t i = 0; i < circles.size(); i++ )
  {
      cv::Point center(cvRound(circles[i][0]), cvRound(circles[i][1]));
       int radius = cvRound(circles[i][2]);
        //circle center
       cv::circle( tar1, center, 3, cv::Scalar(0,255,0), -1, 8, 0 );
        // circle outline
        cv::circle( tar1, center, radius, cv::Scalar(0,0,255), 3, 8, 0 );
   }

    /// Show your results
   cv::namedWindow( "Hough Circle Transform ", CV_WINDOW_KEEPRATIO);
    cv::imshow( "Hough Circle Transform ", mat );
